...
test_scanner() {
  ...
  if [ "$((RANDOM % 2))" = 1 ]; then 
    info "Using project properties file to configure SQ URL and .sonar location"
    ...
    info "Using env variable to configure SQ URL and .sonar location"
    args="--env SONAR_HOST_URL=http://${sonarqube_container_name}:9000 --env SONAR_USER_HOME=/usr/.sonar"
  fi


  docker run --network="$network" --name="$container_name" --user="$(id -u):$(id -g)" -it -v "$PWD/$container_name/sonarqube-scanner:/usr/src" -v "${scanner_cache}:/usr/.sonar" $args "$image"
